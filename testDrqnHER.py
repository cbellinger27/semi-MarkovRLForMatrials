...
tmpState = np.ndarray(shape=(0,4)) 
for i in range(trace_length):
	tmpState = np.concatenate((tmpState, np.array([0,0,env.xDestination, env.yDestination]).reshape(1,4)))

agent = bdrqn.BasicDRQN(env=env, numStatesX=(env.xMax-env.xMin)+1, numStatesY=(env.xMax-env.yMin)+1, stateWithGoal=True, numActions=4,trace_length=trace_length)
episodeBuffer = []
...
while episode <= numEpisodes:
	...
	tmpState = np.concatenate((tmpState[1:,:].reshape(trace_length-1,4), np.array(np.append(old_state, (env.xDestination, env.yDestination))).reshape(1,4)))
	herTmpOldState = tmpState
	herTmpNewState = tmpState
	...
	herTmpNewState = np.concatenate((herTmpNewState[1:,:].reshape(trace_length-1,4), np.array(np.append(new_state, (env.xDestination, env.yDestination))).reshape(1,4)))
	if withHer == True and done == False and epSteps < 10000 and np.random.uniform() > RND_GOAL_PROB:
		herTmpOldState[:,2:] = new_state
		herTmpNewState[:,2:] = new_state
		herTmpNewState = np.concatenate((herTmpNewState[1:,:].reshape(trace_length-1,4), np.array(np.append(new_state, new_state)).reshape(1,4)))
		episodeBuffer.append(np.reshape(np.array([herTmpOldState,action,1,herTmpNewState,done]),[1,5]))
	else:
		# print(herTmpOldState)
		episodeBuffer.append(np.reshape(np.array([herTmpOldState,action,reward,herTmpNewState,done]),[1,5]))
...
plt.plot(np.arange(len(mean_steps_per_episode[0])), mean_steps_per_episode[0], color="blue", linestyle="--", label="DRQN+HER")
plt.legend()
plt.show()
