...
stateSpaceSize = (((env.yMax-env.yMin)+1) + ((env.yMax-env.yMin)+1))*(histSz)
...
agent = bdqn.BasicDQN(env=env, numStatesX=(env.xMax-env.xMin)+1, numStatesY=(env.yMax-env.yMin)+1, stateSpaceSize=stateSpaceSize, numActions=4,stateHistSize=histSz) # 4 actions plus the 2-d goal state
...
while episode < numEpisodes:
	...
	new_state = np.array(hist).flatten()
	agent.update(old_state, new_state, action, reward, done)
	if done or epSteps > 10000:
		...
		if episode % 50 == 0: # TEST PERFORMANCE 
			...
			while not done and tstStps < 10000:
				tstStps += 1
				savedCurTrajectory = np.concatenate((savedCurTrajectory,np.array(new_state).reshape(1,2)))
				old_state = new_state # Store current state
				action = agent.get_next_action(old_state) # Query agent for the next action
...
plt.plot(np.arange(len(mean_steps_per_episode[0])), mean_steps_per_episode[0], color="blue", linestyle="--", label="DQN")
plt.legend()
plt.show()
